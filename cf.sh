...
then 
	...
	then 
		...
		temp=($(curl -x $anycast:80 --retry 1 -s http://$domain/cdn-cgi/trace --connect-timeout 2 --max-time 3)) fi 
	else 
if 
	[ $tls == 1 ] 
	then 
...
if 
[ $(echo ${temp[@]} | sed -e 's/ /\n/g' | grep colo= | wc -l) == 0 ] then publicip 
= 
	get timeout 
	colo=get timeout 
else 
	publicip=$( echo ${temp[@]} | sed -e 's/ /\n/g' | grep ip= | cut -f 2- -d'=') colo=$(grep 
...
echo "Total time spent $[$endtime- $starttime] seconds" 
} 
	...
	do
		...
		then 
			...
			then 
				...
				rsp=$(curl -x [$ip]:80 http: //$domain/cdn-cgi/trace -o /dev/null -s --connect-timeout 1 --max-time 3 -w %{time_connect}_%{http_code}) fi if [ $(echo 
			$ 
			rsp | awk -F_ '{print $2}') != 200 ] 
			then 
				avgms=0 
				n=1 
				break 
...
for i in `cat log.txt | tr '\r' '\n' | awk '{print $NF}' | sed '1,3d;$d' | grep k | ` 
do 
	k=$i 
	k=$[$k*1024] 
	echo $k >> speed.txt 
...
do 
	...
	do 
		...
		then 
			...
			while true 
			do 
				if [ $(echo ${temp[@]} | sed -e 's/ /\n/g' | sort -u | wc -l) -ge $iplist ] then break 
				else 
					for 
				i 
					in `awk 'BEGIN{srand()} {print rand()"\t"$0}' $filename | sort -n | awk '{print $2} NR=='$[$iplist-$(echo ${temp[ @]} | sed -e 's/ /\n/g' | sort -u | wc -l)]' {exit}' | awk -F: '{print $1":"$2":"$3}' ` 
					do 
						temp[$n]=$(echo $i:$(printf '%x\n' $(($RANDOM*2+$RANDOM%2))):$(printf '%x\n' $( ($RANDOM*2+$RANDOM%2))):$(printf '%x\n' $(($RANDOM*2+$RANDOM%2))):$(printf '%x\n' $( ($RANDOM*2+$RANDOM%2))):$(printf '%x\n' $(($RANDOM*2+$RANDOM%2)))) 
						n=$[$n+1] 
					done 
...
echo "Testing $ip port $port" 
speed_download=$(curl --resolve $domain:$port:$ip https://$domain:$port/$file -o /dev/null --connect-timeout 5 --max-time 15 -w %{speed_download} | awk -F\. '{printf ("%d\ n",$1/1024)}') 
} 
...
function datacheck(){ 
clear 
echo "If the download of the following files fails, you can manually visit the URL to download and save to the same level directory" 
...
do 
	...
	then 
		echo "Download speed test file from server Address url.txt" 
		curl --retry 2 -s https://www.baipiao.eu.org/cloudflare/url -o url.txt 
	elif [ ! -f "ips-v4.txt" ] 
	then 
		echo "From server Download IPV4 data ips-v4.txt" 
		curl --retry 2 -s https://www.baipiao.eu.org/cloudflare/ips-v4 -o ips-v4.txt
	elif [ ! -f "ips-v6.txt" ] 
	then 
		echo "Download IPV6 data ips-v6.txt from server" 
